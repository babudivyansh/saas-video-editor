    ...
    <div className="flex flex-col gap-0 text-xs">
      ...
      <Section label="Font">
        ...
        <Row label="Size">
          ...
          <span className="w-10 text-right tabular-nums" style={{ color: "#5a6170" }}>{data.fontSize}px</span>
        </Row>
      ...
      <Section label="Color">
        <Row label="Text">
          ...
          <span style={{ color: "#5a6170" }}>{data.color}</span>
        </Row>
      </Section>
      ...
      <Section label="Position">
        <Row label="X">
          ...
          <span className="w-10 text-right tabular-nums" style={{ color: "#5a6170" }}>{Math.round(data.posX * 100)}%</span>
        </Row>
        <Row label="Y">
          ...
          <span className="w-10 text-right tabular-nums" style={{ color: "#5a6170" }}>{Math.round(data.posY * 100)}%</span>
        </Row>
      </Section>
      ...
      <Section label="Stroke">
        <div className="flex items-center justify-between mb-1">
          <span style={{ color: "#5a6170" }}>Enable</span>
        ...
        {data.stroke && (
          <>
            ...
            <Row label="Width">
              ...
              <span className="w-10 text-right tabular-nums" style={{ color: "#5a6170" }}>{data.strokeWidth}px</span>
            </Row>
          </>
        )}
      </Section>
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs w-12 flex-shrink-0" style={{ color: "#5a6170" }}>{label}</span>
...
function ToggleBtn({ active, onClick, children, italic }: { active: boolean; onClick: () => void; children: React.ReactNode; italic?: boolean }) {
  return (
    <button
      ...
      style={{
        background: active ? "#e8edff" : "#f6f7f9",
        border: `1px solid ${active ? "#335cff" : "#e4e7ec"}`,
        color: active ? "#335cff" : "#5a6170",
