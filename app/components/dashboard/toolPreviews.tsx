...
export function AutoClipPreview() {
  return (
    <div className="h-[160px] bg-gradient-to-br from-tint-violet to-tint-fuchsia flex items-center justify-center gap-3 px-4 overflow-hidden">
      ...
      <div className="bg-panel rounded-xl shadow-lg p-2.5 min-w-[112px]">
        <p className="text-fg-subtle text-[9px] font-medium mb-1.5">How many viral clips do you want to cut</p>
        <div className="flex gap-0.5 mb-2">
          {[1, 2, 3, 4, 5].map(n => (
            <span key={n} className={`w-5 h-5 rounded text-[9px] font-bold flex items-center justify-center ${n === 5 ? "bg-brand text-on-primary" : "bg-surface-3 text-fg-muted"}`}>{n}</span>
          ))}
        </div>
        {["Stream ✓", "Podcast", "Interview", "Lecture"].map(t => (
          <div key={t} className="flex items-center gap-1 mb-0.5">
            <div className={`w-2 h-2 rounded-sm border ${t.includes("✓") ? "bg-brand border-brand" : "border-line-strong"}`} />
            <span className="text-[9px] text-fg-muted">{t.replace(" ✓", "")}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="h-[160px] bg-gradient-to-br from-tint-blue to-tint-violet flex items-center justify-center gap-3 px-4 overflow-hidden">
      <div className="flex gap-1">
        {["from-surface-3 to-black", "from-panel-raised to-surface-2", "from-line-strong to-surface-3"].map((g, i) => (
  ...
  return (
    <div className="h-[140px] bg-gradient-to-br from-gray-50 to-tint-blue flex items-center justify-center gap-4 px-6 overflow-hidden">
      {[true, false].map((hasSub, i) => (
        <div key={i} className="relative w-[88px] h-[108px] rounded-xl overflow-hidden shadow border border-line">
          <div className="w-full h-full bg-gradient-to-b from-surface-3 to-black flex items-end justify-center p-2">
            {hasSub
...
export function ImageGenPreview() {
  return (
    <div className="h-[212px] bg-surface-2 flex flex-col gap-2.5 p-4 overflow-hidden">
      <div className="bg-panel rounded-lg border border-line px-3 py-2.5 text-[11px] text-fg-muted shadow-sm">a small, adorable cat with bright green eyes…</div>
      <div className="flex gap-1.5">
        <span className="text-[9px] font-semibold text-fg-muted border border-line bg-panel rounded-md px-2 py-1">⊞ View Presets</span>
        ...
        <span className="text-[9px] font-semibold text-fg-muted border border-line bg-panel rounded-md px-2 py-1">DALL·E 3 ▾</span>
      </div>
...
export function VoiceoverPreview() {
  ...
  return (
    <div className="h-[212px] bg-surface-2 flex flex-col gap-2 p-4 overflow-hidden">
      <div className="flex gap-1.5">
        {narrators.map((x, i) => (
          <div key={i} className="flex-1 bg-panel rounded-lg border border-line px-2 py-1.5 shadow-sm">
            <div className="flex items-center gap-1 mb-1">
              <span className="w-2 h-2 rounded-full bg-brand" />
              <span className="text-[9px] font-bold text-fg truncate">{x.n}</span>
            </div>
            <p className="text-[8px] text-fg-subtle">{x.t}</p>
          </div>
        ))}
      </div>
      <div className="bg-panel rounded-lg border border-line p-2.5 flex-1 shadow-sm">
        <p className="text-[9px] font-bold text-fg-muted mb-1.5">⌨ Type your script</p>
...
export function SpeechEnhancerPreview() {
  ...
  return (
    <div className="h-[212px] bg-surface-2 flex flex-col gap-3 p-4 justify-center overflow-hidden">
...
export function YouTubeDownloaderPreview() {
  return (
    <div className="h-[212px] bg-surface-2 flex flex-col gap-3 p-4 justify-center overflow-hidden">
      <div className="bg-panel rounded-xl border border-line px-3 py-2.5 flex items-center gap-2 shadow-sm">
        ...
        <div className="h-1.5 rounded-full bg-surface-3 flex-1" />
      </div>
        ...
        <div className="flex gap-2.5 p-2">
          <div className="w-16 h-10 rounded-lg bg-gradient-to-br from-red-400 to-orange-500 flex items-center justify-center flex-shrink-0">
            <span className="w-4 h-4 rounded-full bg-white/80 flex items-center justify-center">
              <svg viewBox="0 0 10 10" fill="currentColor" className="w-2 h-2 text-error"><polygon points="3,1 9,5 3,9"/></svg>
            </span>
          </div>
          <div className="flex-1 min-w-0 py-0.5">
            <div className="h-1.5 rounded-full bg-surface-3 w-full mb-1.5" />
            <div className="h-1.5 rounded-full bg-surface-3 w-2/3" />
          </div>
        </div>
      </div>
      <div className="flex gap-1.5">
...
export function BgRemoverPreview() {
  return (
    <div className="h-[212px] bg-surface-2 flex items-center justify-center gap-3 p-4 overflow-hidden">
      {/* Original with background */}
      <div className="flex-1 rounded-xl overflow-hidden border border-line bg-panel flex flex-col items-center justify-center p-2 gap-1.5 h-full max-h-44">
        ...
        <p className="text-[9px] text-fg-subtle mt-1">Original</p>
      </div>
      {/* Arrow */}
      ...
      {/* Result transparent */}
      <div
        className="flex-1 rounded-xl overflow-hidden border border-line flex flex-col items-center justify-center p-2 gap-1.5 h-full max-h-44"
        style={{
      ...
      >
        ...
        <p className="text-[9px] text-fg-subtle mt-1">No Background</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="h-[212px] bg-surface-2 flex items-center justify-center gap-3 p-4 overflow-hidden">
      ...
      <div className="flex-1 rounded-xl overflow-hidden border border-line bg-panel flex flex-col items-center justify-center p-2 gap-1.5 h-full max-h-44">
        ...
        <p className="text-[9px] text-fg-subtle mt-1">Character</p>
      </div>
      {/* Arrow */}
      <svg viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" className="w-5 h-5 flex-shrink-0"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
      {/* Target image */}
...
export function VerticalSplitPreview() {
  ...
  return (
    <div className="h-[212px] bg-surface-2 flex flex-col items-stretch gap-0 overflow-hidden">
      {/* Top half: user video */}
      <div className="flex-1 bg-black flex items-center justify-center gap-3 border-b border-line">
  ...
  return (
    <div className="h-[212px] bg-surface-2 flex gap-2.5 p-4 overflow-hidden">
      <div className="w-1/2 flex flex-col gap-2">
        <div className="w-7 h-7 rounded-lg grad-brand flex items-center justify-center text-on-primary text-xs">✦</div>
        {["Idea topic", "Select a tone", "Target Audience"].map((l, i) => (
          <div key={i} className="bg-panel rounded-md border border-line px-2 py-1.5 shadow-sm">
            <p className="text-[8px] text-fg-subtle">{l}</p>
          </div>
        ))}
        <span className="text-[9px] font-semibold text-white bg-gray-300 rounded-md px-2 py-1.5 text-center">Generate</span>
      </div>
      <div className="w-1/2 bg-panel rounded-lg border border-line p-2.5 shadow-sm">
        <p className="text-[8px] text-fg-subtle mb-2">Your idea will appear here</p>
