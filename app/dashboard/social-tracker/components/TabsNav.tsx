          ...
          return (
            <li key={tab.href}>
              <Link
                ...
                // MITIGATION, not a fix. In production every one of these six
                // prefetches comes back 503 (locally the same request is a clean
                // 307), so the burst is almost certainly being shed by the host
                // rather than generated by us — six concurrent RSC requests, on
                // every page load, each a force-dynamic route that queries the
                // database. Until that is diagnosed the prefetches deliver
                // nothing and cost six requests, so stop firing them.
                //
                // Little is given up meanwhile: these routes are force-dynamic,
                // so App Router prefetch would only have fetched as far as
                // loading.tsx anyway.
                prefetch={false}
