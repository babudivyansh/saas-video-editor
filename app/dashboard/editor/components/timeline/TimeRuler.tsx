...
export default function TimeRuler({ totalSeconds, zoom }: { totalSeconds: number; zoom: number }) {
  ...
  return (
    <div className="relative h-6 cursor-pointer select-none border-b border-card-border bg-surface">
      {ticks.map((t) => (
        <div key={t} className="absolute bottom-0 top-0 flex items-end" style={{ left: t * zoom }}>
          <span className="h-2 w-px bg-ink-soft/40" />
          <span className="mb-0.5 ml-1 text-[9px] text-ink-soft">{fmt(Math.round(t * 100) / 100)}</span>
        </div>
      ))}
    </div>
  );
