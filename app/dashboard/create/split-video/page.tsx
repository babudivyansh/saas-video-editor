...
// Thumbnails/metadata below reference gameplay-cdn.com ids, but the actual
// render-time background videos are served from our own S3 bucket (the public
// gameplay CDN is not a reachable source). Each catalogue title maps to one
// hosted clip; override the base with NEXT_PUBLIC_BACKGROUNDS_BASE if needed.
...
function GeneratingOverlay({ status, videoUrl, error, onReset }: { status: GenerateStatus; videoUrl: string | null; error: string | null; onReset: () => void }) {
  ...
  if (status === "completed" && videoUrl) {
    return (
      <div className="mt-4 mx-8 mb-8 rounded-[28px] bg-surface-2 border border-line flex items-center justify-center" style={{ minHeight: "calc(100vh - 200px)" }}>
    ...
    <div className="px-8 pt-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-fg">Split Screen Video</h1>
        </div>
      </div>
        ...
        {fileName ? (
          ...
          >
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-md bg-surface-2 px-4 py-2 text-sm font-medium text-white transition-colors hover:opacity-90 h-10"
            >
              Browse File
            </button>
          </div>
...
function BgCard({ b, isSel, onSelect }: { b: typeof BACKGROUNDS[0]; isSel: boolean; onSelect: () => void }) {
  ...
  return (
    <div
      ...
      className="relative overflow-hidden rounded-lg border bg-panel cursor-pointer"
    ...
    >
      ...
      <div className="space-y-1 p-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-fg">{b.title}</h3>
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-xs text-fg-muted">{b.author}</p>
          </div>
        </div>
        <div className="flex flex-col items-center gap-2">
          <div className="flex w-full flex-row items-start gap-1">
            <span className="rounded-md bg-surface-3 px-1.5 py-0.5 text-xs text-fg">{b.mins}</span>
            <span className="rounded-md bg-surface-3 px-1.5 py-0.5 text-xs text-fg">{b.size}</span>
            <span className="rounded-md bg-surface-3 px-1.5 py-0.5 text-xs text-fg">Free</span>
          </div>
          ...
          <a
            ...
            className="w-full rounded-lg border border-line p-2 text-center text-sm text-fg-muted hover:text-fg transition-colors block"
          >
  ...
  return (
      <main className="h-full overflow-y-auto bg-panel">
        ...
        {isGenerating ? (
          ...
          <div className="mt-4 mx-8 mb-8 rounded-[28px] bg-surface-2 border border-line" style={{ minHeight: "calc(100vh - 200px)" }}>
            {stepIndex === 0 && (
              <UploadStep
                onFile={handleFile}
                onAsset={handleAsset}
            ...
            {stepIndex === 2 && (
              <SubtitleTemplateStep value={captionTemplateId} onChange={setCaptionTemplateId} mode={subMode} onModeChange={setSubMode} />
            )}
          </div>
        )}
      </main>
