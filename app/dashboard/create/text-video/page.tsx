...
function VoiceSettingsPanel({ settings, onChange, language, onLanguageChange }: {
  ...
  return (
    <div className="flex flex-col gap-5">
      <h3 className="text-base font-bold text-fg">Language & Voice Settings</h3>
      {/* Language dropdown */}
      <select
        value={language}
        onChange={e => onLanguageChange(e.target.value)}
        className="w-full rounded-lg border border-line px-3 py-2 text-sm bg-panel text-fg focus:outline-none"
      >
      ...
      {sliders.map(({ key, label }) => (
        <div key={key}>
          <div className="flex items-center gap-1 mb-2">
            ...
            <button className="text-fg-subtle hover:text-fg-muted"><IcInfo /></button>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-fg-subtle w-6">Low</span>
            <input
            ...
            />
            <span className="text-xs text-fg-subtle w-8 text-right">High</span>
          </div>
        </div>
      ))}
      {/* Reset */}
...
function TvVoiceCard({ voice, selected, onSelect }: { voice: Voice; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <div
      onClick={onSelect}
      className={`flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${selected ? "border-purple-500 bg-panel" : "border-line bg-panel hover:border-line-strong"}`}
...
function AiScriptModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div
        className="bg-panel rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col gap-4"
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-fg-subtle hover:text-fg-muted text-xl leading-none">✕</button>
        </div>

        <div>
          <label className="text-[11px] font-bold text-fg-muted uppercase tracking-wide block mb-1.5">
            What&apos;s the scenario?
          </label>
          <textarea
            value={prompt}
            ...
            rows={3}
            maxLength={500}
            className="w-full border border-line rounded-xl px-3 py-2.5 text-sm text-fg focus:outline-none focus:border-blue-400 resize-none"
          />
          <p className="text-[10px] text-fg-subtle mt-0.5 text-right">{prompt.length}/500</p>
        </div>

        <div>
          <label className="text-[11px] font-bold text-fg-muted uppercase tracking-wide block mb-2">
            Tone
          </label>
        ...
        {err && <p className="text-sm text-error font-semibold">{err}</p>}
      ...
      <main className="h-full flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-line flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg border border-line flex items-center justify-center font-bold text-fg text-lg">T</div>
            <h1 className="text-xl font-bold text-fg">Text Video</h1>
          </div>
        </div>
            ...
            {STEPS.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 text-sm font-semibold ${i === stepIndex ? "text-blue-600" : i < stepIndex ? "text-fg-muted hover:text-fg cursor-pointer" : "text-fg-subtle cursor-default"}`}
                >
                  <span className={`w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold ${i === stepIndex ? "bg-blue-600 text-white" : i < stepIndex ? "bg-surface-3 text-fg-muted" : "bg-surface-3 text-fg-subtle"}`}>
                    {i + 1}
                  </span>
                  {s.label}
                </button>
          ...
          <div className="flex items-center gap-2">
            {stepIndex > 0 && (
              <button onClick={() => goTo(stepIndex - 1)}
                className="flex items-center gap-1.5 text-sm font-semibold text-fg-muted hover:text-fg px-4 py-2 rounded-lg border border-line hover:bg-surface-2 transition-colors">
                <IcChevronLeft /> Back
              </button>
            )}
            {stepIndex < STEPS.length - 1 ? (
              <button onClick={() => canNext && goTo(stepIndex + 1)}
                className={`flex items-center gap-1.5 text-sm font-semibold px-5 py-2 rounded-lg transition-colors ${canNext ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-surface-3 text-fg-subtle cursor-not-allowed"}`}>
                Next <IcChevron />
              </button>
            ) : (
              <button
                ...
                className={`flex items-center gap-1.5 text-sm font-bold px-5 py-2 rounded-lg transition-colors ${genStatus === "creating" || genStatus === "rendering" ? "bg-surface-3 text-fg-subtle cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 text-white"}`}>
        ...
        {genStatus === "failed" && genError && (
          <div className="flex items-center gap-3 px-6 py-3 bg-error/10 border-b border-error/40">
            <span className="text-sm font-semibold text-error">{genError}</span>
            <button onClick={reset} className="ml-auto text-xs text-fg-subtle hover:text-fg-muted">Dismiss</button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-hidden flex">
          ...
          {stepIndex === 0 && (
            <>
              {/* Left: Editor */}
              <div className="flex-1 flex flex-col border-r border-line overflow-hidden">
                {/* Panel header */}
                <div className="flex items-center justify-between px-5 py-3 border-b border-line">
                  <h2 className="text-sm font-bold text-fg">Text Script</h2>
                  <div className="flex items-center gap-1.5">
                    <button onClick={swapMessages}
                      className="flex items-center gap-1.5 text-xs font-semibold text-fg-muted border border-line rounded-lg px-3 py-1.5 hover:bg-surface-2 transition-colors">
                      <IcSwap /> Swap
                    </button>
                    ...
                    <button onClick={loadSample}
                      className="text-xs font-semibold text-fg-muted border border-line rounded-lg px-3 py-1.5 hover:bg-surface-2 transition-colors">
                      Sample Data
                    </button>
                    <button
                ...
                <div className="px-5 py-4 border-b border-line space-y-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-10 h-10 rounded-full bg-surface-3 flex items-center justify-center overflow-hidden flex-shrink-0 cursor-pointer border border-line"
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-fg">Upload Profile Picture</p>
                      <p className="text-[11px] text-fg-subtle">Recommended: 400px × 400px, PNG or JPEG</p>
                    </div>
                    <button onClick={() => fileInputRef.current?.click()}
                      className="flex items-center gap-1.5 text-xs font-semibold text-fg-muted border border-line rounded-lg px-3 py-1.5 hover:bg-surface-2 transition-colors flex-shrink-0">
                      <IcUpload /> Upload
                    </button>
                  ...
                  {avatarError && <p className="text-xs text-error font-medium">{avatarError}</p>}
                  <div>
                    <label className="text-[11px] font-bold text-fg-muted uppercase tracking-wide block mb-1">Enter a name</label>
                    <input value={name} onChange={e => setName(e.target.value)}
                      className="w-full border border-line rounded-lg px-3 py-2 text-sm text-fg focus:outline-none focus:border-blue-400"
                ...
                <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
                  {messages.length === 0 && (
                    <p className="text-sm text-fg-subtle text-center py-8">Add messages with the buttons below</p>
                  )}
                  {messages.length > 0 && (
                    <p className="text-[11px] text-fg-subtle text-right">{messages.length}/{MAX_MESSAGES} messages</p>
                  )}
                  {messagesError && <p className="text-xs text-error font-medium">{messagesError}</p>}
                  {messages.map(m => (
                    <div key={m.id} className={`flex items-center gap-2 ${m.type === "sender" ? "flex-row-reverse" : ""}`}>
                      ...
                      <input
                        ...
                        className="flex-1 border border-line rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400"
                      />
                      <button onClick={() => removeMessage(m.id)} className="text-fg-subtle hover:text-error transition-colors flex-shrink-0">
                        <IcTrash />
                      </button>
                    </div>
                  ))}
          ...
          {stepIndex === 1 && (
            <>
              <div className="flex-1 overflow-y-auto px-6 pt-6 pb-10">
                <h2 className="text-lg font-bold text-fg mb-5">Choose theme</h2>
          ...
          {stepIndex === 2 && (
            <div className="flex-1 overflow-y-auto px-6 pt-6 pb-10">
              ...
              <p className="text-sm text-fg-muted mt-1 mb-5">
                <span className="font-semibold text-fg">Tip:</span> You can replace the background video after generation in the editor.
              </p>
                ...
                {BACKGROUNDS.map((b, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setSelectedBg(i)}
                      ...
                      {/* Thumbnail */}
                      <div className="relative bg-surface-3" style={{ paddingBottom: "177%" }}>
                        {isSel && (
                      ...
                      <div className="p-3 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm font-semibold text-fg truncate">{b.title}</h3>
                          <div className="flex items-center gap-1">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={b.authorImg} alt={b.author} className="h-4 w-4 rounded-full object-cover" />
                            <span className="text-[10px] text-fg-muted truncate max-w-[60px]">{b.author}</span>
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={e => e.stopPropagation()}
                          className="w-full mt-1 py-1.5 rounded-lg border border-line text-[11px] font-semibold text-fg-muted hover:bg-surface-2 transition-colors"
                        >
          ...
          {stepIndex === 3 && (
            <div className="flex-1 overflow-y-auto">
              <div className="border border-line rounded-2xl mx-6 my-4 overflow-hidden">
                <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-line">
                  ...
                  <div className="p-5">
                    ...
                    {/* Background Music */}
                    <div className="mt-6">
                      <h4 className="text-sm font-bold text-fg mb-3">Background Music</h4>
                        ...
                        {BACKGROUND_MUSIC.map((m, i) => {
                          ...
                          return (
                            <div key={i} onClick={() => setSelectedMusic(i)}
                              className={`px-3 py-2.5 rounded-xl border cursor-pointer transition-all ${isSel ? "border-purple-500 bg-panel" : "border-line hover:border-line-strong"}`}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-semibold text-fg flex-1 truncate">{m.name}</span>
                                {m.duration && <span className="text-[10px] text-fg-subtle">{m.duration}</span>}
                                {i > 0 && <IcMusic />}
                              </div>
                              {i > 0 && (
                              ...
                              {i === 0 && <p className="text-[10px] text-fg-subtle mt-1 leading-relaxed">{m.desc}</p>}
                            </div>
                          );
                        })}
                      </div>
