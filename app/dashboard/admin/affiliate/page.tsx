...
export default function AdminAffiliatePage() {
  ...
  // Affiliates with available balance ≥ ₹500
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Affiliate Program Admin</h1>
                ...
                return (
                  <tr key={a.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <input
                        type="number"
                        defaultValue={(a.commissionRate * 100).toFixed(0)}
                        min={0} max={100}
                    ...
                    <td className="px-4 py-3">
                      {a.status === "active" ? (
                        <button onClick={() => updateAffiliate(a.id, { status: "suspended" })}
                          className="text-xs text-yellow-600 hover:underline">Suspend</button>
                      ) : (
                        <button onClick={() => updateAffiliate(a.id, { status: "active" })}
                ...
                {filteredCommissions.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 flex gap-2">
                      {c.status === "pending" && (
                        <button onClick={() => commissionAction(c.id, "release")}
                          className="text-xs text-blue-600 hover:underline">Release</button>
                      )}
                      {(c.status === "pending" || c.status === "available") && (
                        <button onClick={() => commissionAction(c.id, "reject")}
